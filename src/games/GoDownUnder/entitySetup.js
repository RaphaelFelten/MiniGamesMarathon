...
export function createObstacles(obstacleImage, speed, gameContext) {  
    for (let i = 0; i < nObstaclesOnScreen / 2; i++) {
        spawnObstacle(obstacleImage, speed, (i + 1) * gameContext.canvas.height / nObstaclesOnScreen * 2, gameContext);
    }
}
...
export function spawnObstacle(obstacleImage, speed, y, gameContext) {
    ...
    const h = Math.floor(gameContext.canvas.height / nObstaclesOnScreen / 1.5);
